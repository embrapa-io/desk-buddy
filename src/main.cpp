...
static void buildOverview(lv_obj_t* scr) {
  ...
  lv_obj_align(grid, LV_ALIGN_TOP_LEFT, 0, 28);
  flat(grid);
  ...
  // 3 tiles de grupo + 1 tile geral
  for (int t = 0; t < 4; t++) {
    lv_obj_t* tile = lv_obj_create(grid);
    lv_obj_set_size(tile, 110, 88);
    styleCard(tile);
    ...
    if (t < G_COUNT) {
      lv_label_set_text(lbl, GRP_LABEL[t]);
      ...
      lv_obj_align(ovVal[t], LV_ALIGN_CENTER, 0, 2);
...
static lv_obj_t* buildListPage(lv_obj_t* scr, int gi, const char* title) {
  ...
  lv_obj_set_size(box, SCR_W - 64 - 16, SCR_H - 38);
  lv_obj_align(box, LV_ALIGN_TOP_LEFT, 0, 30);
  flat(box);
...
void loop() {
  ...
  // fetch periódico
  if (conn && (lastFetch == 0 || millis() - lastFetch > REFRESH_MS)) {
    lastFetch = millis();
    if (fetchGatus()) {
      refreshUI();
